...
def load_matrix_config_from_env() -> dict[str, Any] | None:
    ...
    Reads the environment variables specified by the module-level mapping and converts present values into a dictionary keyed by configuration keys. Useful for injecting Matrix passwords via Kubernetes Secrets.
...
def apply_env_config_overrides(config: dict[str, Any] | None) -> dict[str, Any]:
    ...
    keys "meshtastic", "logging", "matrix", and "database" when present; existing keys in those sections are preserved.
