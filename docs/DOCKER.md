...
- **Data Directory**: `~/.mmrelay/` (mounted to `/data`). This directory on your host will contain subdirectories for the database (`mmrelay.db`), logs (`logs/`), plugins (`plugins/`), E2EE store (`store/`), and credentials (`credentials.json`).
