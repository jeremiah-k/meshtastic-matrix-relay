...
# check_doc_files scans files from DOC_FILES for occurrences of tokens in PATTERNS and reports any matches that are not contained within a fenced code block immediately following the <!-- MMRELAY_ALLOW_LEGACY_EXAMPLE --> marker.
check_doc_files() {
...
# selftest creates a temporary markdown file with marker-allowed fenced blocks and forbidden tokens, runs check_doc_files to verify exactly two forbidden-pattern errors are reported, restores PATTERNS/DOC_FILES/ERROR_FOUND, cleans up temporary files, and returns 0 on success or 1 on failure.
selftest() {
	local TEST_FILE
