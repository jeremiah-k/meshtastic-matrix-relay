...
class TestAsyncHelperEdgeCases(unittest.TestCase):
    ...
    @patch("mmrelay.meshtastic_utils.logger")
    @patch("mmrelay.meshtastic_utils.event_loop", None)
    @patch("mmrelay.meshtastic_utils.running")
    def test_submit_coro_ultimate_fallback_runtime_error(
        self, mock_running, mock_logger
    ):
        ...
        # Should return a Future with the exception set
        self.assertIsNotNone(result)
        self.assertIsInstance(result, asyncio.Future)
        with self.assertRaises(RuntimeError):
            result.result(timeout=1)

        # Should log the ultimate fallback message
    ...
    @patch("mmrelay.meshtastic_utils.logger")
    @patch("mmrelay.meshtastic_utils.event_loop", None)
    @patch("asyncio.new_event_loop", side_effect=OSError("test error"))
    def test_submit_coro_ultimate_fallback_os_error(self, mock_logger):
