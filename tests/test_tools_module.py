...
def test_get_sample_config_path_uses_package_resource_files(tmp_path: Path) -> None:
    package_root = tmp_path / "fake-tools"
...
def test_get_service_template_path_uses_package_resource_files(tmp_path: Path) -> None:
    package_root = tmp_path / "fake-tools"
    traversable = MagicMock()
