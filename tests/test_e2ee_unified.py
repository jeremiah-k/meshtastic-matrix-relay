...
class TestUnifiedE2EEStatus(unittest.TestCase):
    ...
    def test_e2ee_incomplete_missing_deps(self, mock_exists):
        ...
        with patch('builtins.__import__', side_effect=ImportError("No module named 'olm'")):
            ...
            self.assertIn("E2EE dependencies not installed (python-olm)", status["issues"])
...
class TestRoomListFormatting(unittest.TestCase):
    ...
    def test_room_list_e2ee_ready(self):
        ...
        rooms = {
            "!encrypted:test.org": MockRoom("!encrypted:test.org", "Encrypted Room", encrypted=True),
            "!plaintext:test.org": MockRoom("!plaintext:test.org", "Plaintext Room", encrypted=False),
        }
    ...
    def test_room_list_e2ee_disabled(self):
        ...
        self.assertIn("   ⚠️ Encrypted Room - Encrypted (E2EE disabled - messages will be blocked)", room_lines)
    ...
    def test_room_list_e2ee_unavailable(self):
        ...
        self.assertIn("   ⚠️ Encrypted Room - Encrypted (E2EE unavailable on Windows)", room_lines)
...
class TestEncryptionWarnings(unittest.TestCase):
    ...
    def test_warnings_for_encrypted_rooms_disabled(self):
        ...
        rooms = {
            "!encrypted1:test.org": MockRoom("!encrypted1:test.org", "Room 1", encrypted=True),
            "!encrypted2:test.org": MockRoom("!encrypted2:test.org", "Room 2", encrypted=True),
            "!plaintext:test.org": MockRoom("!plaintext:test.org", "Room 3", encrypted=False),
        }
    ...
    def test_no_warnings_when_ready(self):
        ...
        rooms = {
            "!encrypted:test.org": MockRoom("!encrypted:test.org", "Room 1", encrypted=True),
        }
...
class TestActualEncryptionVerification(unittest.TestCase):
    ...
    def test_encryption_log_detection(self):
        ...
        try:
            ...
            nio_crypto_logger.info("Creating outbound group session for !test:matrix.org")
            nio_crypto_logger.info("Created outbound group session for !test:matrix.org")

            # Verify logs were captured
            self.assertIn("Sharing group session for room !test:matrix.org", log_capture)
            self.assertIn("Creating outbound group session for !test:matrix.org", log_capture)
            self.assertIn("Created outbound group session for !test:matrix.org", log_capture)
    ...
    def test_encrypted_event_detection(self):
        ...
        self.assertEqual(encrypted_event["content"]["algorithm"], "m.megolm.v1.aes-sha2")
        self.assertIn("ciphertext", encrypted_event["content"])
        self.assertGreater(len(encrypted_event["content"]["ciphertext"]), 50)  # Should be substantial
