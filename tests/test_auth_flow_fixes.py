        ...
        with patch("sys.platform", "win32"):
            ...
            with patch("mmrelay.config.get_base_dir", return_value=config_dir):
                ...
                def mock_exists(path):
                    ...
                    Returns False for the credentials file path, True for the configuration directory path, and False for any other path. Uses the outer-scope variables `credentials_path` and `config_dir` (closure) to decide which paths exist.
                    
                    Parameters:
                        path (str): Filesystem path to check.
                    
                    Returns:
                        bool: True if the mock should consider the path present, otherwise False.
                    """
