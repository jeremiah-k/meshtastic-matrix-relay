...
from mmrelay.cli import main, generate_sample_config
...
class TestCLIWindowsErrorHandling(unittest.TestCase):
    ...
    def test_generate_config_windows_error_handling(self, mock_print, mock_get_error, mock_is_windows):
    ...
    @patch("mmrelay.windows_utils.is_windows", return_value=True)
    @patch("builtins.print")
    def test_generate_config_windows_troubleshooting_guidance(self, mock_print, mock_is_windows):
...
class TestCLIAuthLoginEnhancements(unittest.TestCase):
    ...
    def test_auth_login_e2ee_enabled_banner(self, mock_print, mock_load_config):
        ...
        printed_messages = [call.args[0] for call in mock_print.call_args_list if call.args]
        e2ee_banner = any("Matrix Bot Authentication for E2EE" in str(msg) for msg in printed_messages)
    ...
    def test_auth_login_no_e2ee_banner_when_disabled(self, mock_print, mock_load_config):
        ...
        printed_messages = [call.args[0] for call in mock_print.call_args_list if call.args]
        standard_banner = any("Matrix Bot Authentication" in str(msg) and "E2EE" not in str(msg) for msg in printed_messages)
...
class TestCLIE2EEValidation(unittest.TestCase):
    ...
    def test_e2ee_validation_improved_error_message(self, mock_print):
        ...
        error_msg_found = any("E2EE dependencies not installed" in str(msg) for msg in printed_messages)
        guidance_found = any("End-to-end encryption features require additional dependencies" in str(msg) for msg in printed_messages)
        install_cmd_found = any("pipx install 'mmrelay[e2e]'" in str(msg) for msg in printed_messages)
